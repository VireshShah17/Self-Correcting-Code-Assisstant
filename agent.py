...
def run_agent(user_task: str):
    """
        Generator function that runs the self-correcting agent loop.
        Yields dictionary states so the Streamlit UI can consume the live updates.
    """
    # Configuration
    MAX_RETRIES = 3
    attempt = 0
    ...
    messages = [
        SystemMessage(
            content = """
                You are an autonomous, expert Python developer. Your only job is to write Python scripts to solve the user's request. 
    ...
    while attempt < MAX_RETRIES:
        if needs_new_code:
            yield {"status": "generating", "message": f"⏳ Attempt {attempt + 1} of {MAX_RETRIES} running..."}
            ...
            yield {"status": "code_generated", "code": code_string}
        ...
        yield {"status": "validating", "message": f"⏳ Attempt {attempt + 1}: Running Security Scan..."}
        validation = validate_code_ast(code_string)
        if not validation["valid"]:
            yield {"status": "security_error", "error": validation['error']}
        ...
        if execution_result["success"]:
            ...
            yield {"status": "success", "output": execution_result["output"]}
            break
        else:
    ...
    if attempt == MAX_RETRIES:
        yield {"status": "fatal_error", "message": "🛑 AI failed to solve the problem after maximum attempts."}